...
class E_GCL_mask_no_edges(nn.Module):
    def __init__(self, input_nf, output_nf, hidden_nf, act_fn=nn.ReLU(), recurrent=True):
        super(E_GCL_mask_no_edges, self).__init__()
    ...
    def forward(self, h, coord=None, node_mask=None, edge_index=None, edge_attr=None, edge_mask=None, node_attr=None, n_nodes=None):
        # Kanten werden nicht verwendet
        h_new = self.node_model(h)

        # Optional: Recurrent Update (wenn `recurrent=True`)
        if self.recurrent:
            h = h + h_new
        else:
            h = h_new
...
class EGNN_no_edges(nn.Module):
    def __init__(self, in_node_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4):
        super(EGNN_no_edges, self).__init__()
        self.hidden_nf = hidden_nf
        self.device = device
        self.n_layers = n_layers
        ...
        for i in range(0, n_layers):
            self.add_module(f"gcl_{i}", E_GCL_mask_no_edges(hidden_nf, hidden_nf, hidden_nf, act_fn=act_fn, recurrent=True))
    ...
    def forward(self, h0, x=None, edges=None, edge_attr=None, node_mask=None, edge_mask=None, n_nodes=None):
        h = self.embedding(h0)

        for i in range(0, self.n_layers):
            h, _, _ = self._modules[f"gcl_{i}"](h)

        h = self.node_dec(h)
        h = h * node_mask if node_mask is not None else h  # Optional: Node-Mask anwenden
        h = h.view(-1, n_nodes, self.hidden_nf)
        h = torch.sum(h, dim=1)
        pred = self.graph_dec(h)
        return pred.squeeze(1)
